...
class LightTrackingSystem:
    ...
    def process_frame(self, frame, show_debug=True):
        ...
        if pose_result.valid:
            ...
            self._update_measurement_panel(pose_result)
        else:
    ...
    def _update_measurement_panel(self, pose_result, status=None):
        """更新独立的距离与姿态数值窗口，黑底仅使用红色文字。"""
        panel = np.zeros((310, 470, 3), dtype=np.uint8)
        red = (0, 0, 255)
        ...
        y = 88
        for line in lines:
            cv2.putText(panel, line, (28, y), font, 0.68, red, 2)
            y += 42
        self.measurement_panel = panel
